...
save_general_log() {
	...
	mkdir $GENERAL_LOG/asdf_backup
	cp /asdf/* $GENERAL_LOG/asdf_backup/
    ...
    for x in SurfaceFlinger window activity input_method input alarm power battery batterystats; do
...
save_modem_log() {
	cp -R /sdcard/Asuslog/Modem $MODEM_LOG 
	echo "cp -R /sdcard/Asuslog/Modem $MODEM_LOG"
}

save_tcpdump_log() {
	if [ -d "/data/media/0/Asuslog/TcpDump/" ]; then
		cp /data/media/0/Asuslog/TcpDump/* $TCP_DUMP_LOG
