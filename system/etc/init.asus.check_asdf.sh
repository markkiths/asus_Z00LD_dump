...
#for atcmd checksum
		...
		if [ -e /asdf/$version_number ]; then
			md5sum /dev/block/platform/soc.0/7824900.sdhci/by-name/boot > /asdf/md5checksum_now
			chmod 755 /asdf/md5checksum_now
			
		else
			echo 0 > /asdf/$version_number
			chmod 700 /asdf/$version_number
			md5sum /dev/block/platform/soc.0/7824900.sdhci/by-name/boot > /asdf/md5checksum_init
